...
$('.portfolio__content').owlCarousel(owlConfig);

if(window.innerWidth >=768){
  $('.portfolio__content').trigger('destroy.owl.carousel');
}

window.addEventListener('resize', ()=>{
  if(window.innerWidth >=768){
    $('.portfolio__content').trigger('destroy.owl.carousel');
    $('.portfolio__content').classList.remove('owl-carousel');
  }
  else{
    $('.portfolio__content').owlCarousel(owlConfig);
    $('.portfolio__content').classList.add('owl-carousel');
...
//   // Кнопки управления слайдером для мобильной версии
